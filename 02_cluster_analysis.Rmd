...
clustered_mort <- clustered_mort %>%
  cbind(id) %>%
  mutate(mort_cluster_ids = case_when(id == 1 ~ 3,
                          id == 2 ~ 1,
                          id == 3 ~ 2,
                          TRUE ~ id))


colourvalues <- c(c("#264653","#2a9d8f","#e9c46a","#f4a261","#e76f51"))
colourvalues <- c(c("#9EF8EE","#22BABB","#348888","#f4a261","#e76f51"))
colourvalues <- c(c("#e9c46a", "#2a9d8f","#264653","#f4a261","#e76f51"))
...
sankey <- ggplot(sankey_df, aes(x = x,
  ...
  geom_sankey(flow.alpha = 0.82, node.color = 1) +
      scale_fill_manual(values = rev(colourvalues),
...
ggsave(sankey, filename = "sankey.png", width = 20, height = 10, unit = "cm")
...
undermap <- ne_countries(scale = "medium", returnclass = "sf")

countries <- clustered_mort %>%
...
ggplot(clustered_mort %>%
         left_join(geodata)) +
  geom_sf(data = undermap,
          aes(geometry = geometry), fill = "lightgrey", colour = NA, lwd = 0.6) +
  geom_sf(aes(fill = as.factor(mort_cluster_ids), geometry = geometry)) +
  geom_sf(data = geodata_0 %>% filter(id %in% unique(countries$country)),
...
# 
# ggplot(geodata %>% filter(grepl("DE", code_nuts)) %>% left_join(responses)) +
#   geom_sf(aes(fill = responses, geometry = geometry), colour = "black", lwd = 0.6) +
#   scale_fill_manual(values = c("grey", "#ffba08" )) +
#   bbc_style() +
#   theme(panel.grid.major = element_blank(),
#         legend.position = "right",
#         axis.text = element_blank(),
...
ggplot(cluster_weekly_exmort, aes(x = date, y = exmort, group = as.factor(mort_cluster_ids), colour = as.factor(mort_cluster_ids), fill = as.factor(mort_cluster_ids))) +
  # geom_line(data = exmort_weekly_nuts3 %>% 
  #             left_join(clustered_mort), 
  #           aes(x = date, y = exmort, group = code_nuts, colour = as.factor(mort_cluster_ids)),
            # linewidth = 0.15,
            # alpha = 0.2) +
  geom_line(linewidth = 1) +
  geom_point(size = 0) +
  bbc_style() +
  scale_colour_manual(values = colourvalues, guide = "none") +
  scale_fill_manual(values = colourvalues) +
...
table <- cluster_weekly_exmort %>%
  ...
  gt() %>%
  gt_theme_538() %>%
  data_color(columns = Cluster, colors = colourvalues) %>%
...
vars_clust <- read_csv("data/nuts3vars.csv") %>% inner_join(clustered_mort %>% filter(mort_cluster_ids < 4))
...
aov(mort_cluster_ids ~ manufacturing, data = vars_clust) %>% summary()

kruskal.test(mort_cluster_ids ~ manufacturing, data = vars_clust)


ggboxplot(vars_clust, x = "mort_cluster_ids", y = "prod_pp_eur")
...
```{r feature-selection, warning=FALSE}

for_boruta <- vars_clust %>% drop_na() %>% filter(mort_cluster_ids < 4)  %>% select(!code_nuts)


boruta_test <- Boruta(mort_cluster_ids~., data = for_boruta)

plot(boruta_test, xlab = "Features")

getSelectedAttributes(boruta_test, withTentative = F)

boruta_out <- attStats(boruta_test) %>%
...
check_variable_contributions <- function(df) {
  # Assuming 'mort_cluster_ids' is the grouping variable
  grouping_var <- 'mort_cluster_ids'
  
  # Extract names of predictor variables (excluding the grouping variable)
  predictor_vars <- setdiff(names(df), as.factor(grouping_var))
  
  # Create formula for linear model
  formula <- paste(grouping_var, "~", paste(predictor_vars, collapse = " + "))
...
check_variable_contributions(for_boruta)
...
vars_long <- vars_clust %>% 
  pivot_longer(!c(code_nuts, mort_cluster_ids), names_to = "var", values_to = "value") %>%
  drop_na()


ggplot(vars_long, aes(x = as.factor(mort_cluster_ids), y=value, fill=mort_cluster_ids)) +
    geom_violin() +
...
# total baseline mortality per nut i.e. mean value previous five years
pre_pando_mort_clust <- weekly_raw_mort_nuts3 %>%
  left_join(clustered_mort %>% select(code_nuts, mort_cluster_ids)) %>%
  drop_na() %>%
  filter(year %in% c(2015, 2016, 2017, 2018, 2019)) %>%
  group_by(code_nuts, year) %>%
  summarise(year_total = sum(raw_mort)) %>%
  group_by(code_nuts) %>%
  summarise(expected_biannual = 2*mean(year_total))


# total excess mortality per nut
excess_mort_clust <- weekly_raw_mort_nuts3 %>%
  left_join(clustered_mort %>% select(code_nuts, mort_cluster_ids)) %>%
  drop_na() %>%
  filter(year %in% c(2020, 2021),
         week < 53) %>%
  group_by(code_nuts) %>%
  summarise(actual_mort = sum(raw_mort)) %>% 
  left_join(pre_pando_mort_clust) %>%
  mutate(exmort = (actual_mort-expected_biannual)/expected_biannual,
         exmort_number = actual_mort - expected_biannual)


excess_mort_total_clust

excess_mort_clust %>%
  left_join(clustered_mort) %>% 
  ggplot(aes(x=mort_cluster_ids,fill=as.factor(mort_cluster_ids),y=exmort))+
...
excess_mort_clust %>%
  mutate(exmort_num = actual_mort - expected_biannual) %>%
  left_join(clustered_mort) %>%
  group_by(mort_cluster_ids) %>%
  reframe(mean = mean(exmort),
          sd = sd(exmort),
          median= median(exmort),
          IQR= IQR(exmort),
          sum = sum(exmort_num)
          )
